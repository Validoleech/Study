...
class VanillaGradientDescent(BaseDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        """
        l = x.shape[0]
        gradient = (2 / l) * (x.T @ (x @ self.w) - x.T @ y)
        return gradient
...
class StochasticDescent(VanillaGradientDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        # Генерация случайных индексов для батча
        ...
        gradient = (2 / l) * (x_batch.T @ (x_batch @ self.w) - x_batch.T @ y_batch)
        return gradient
...
class Adam(VanillaGradientDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        m_cor = self.m / (1 - self.beta_1 ** self.iteration)
        v_cor = self.v / (1 - self.beta_2 ** self.iteration)

        weight_update = - eta_k * m_cor / (np.sqrt(v_cor) + self.eps)
        self.w += weight_update
